const questionsML = [
  ...
  },
  {
    ...
    options: [
      "A) The set of input features",
      "B) The output the model is trained to predict",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Assigning an input to one of three or more categories",
      "C) Using multiple models to predict a single output",
  ...
  },
  {
    ...
    options: [
      ...
      "B) To provide labeled examples for the model to learn from",
  ...
  },
  {
    ...
    options: [
      "A) A function that preprocesses the data",
      "B) A function that maps inputs to predicted outputs",
  ...
  },
  {
    ...
    options: [
      ...
      "B) The discrepancy between predicted and actual values",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Parametric models have a fixed number of parameters regardless of data size",
      "C) Non-parametric models cannot handle large datasets",
      "D) Non-parametric models do not learn from data"
    ],
    optionsChinese: [
      "A) 参数模型总是更准确",
      "B) 参数模型的参数数量固定，与数据大小无关",
      "C) 非参数模型无法处理大型数据集",
      "D) 非参数模型不从数据中学习"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "A) Memorizing the training data exactly",
  ...
  },
  {
    ...
    options: [
      "A) Predicting whether a patient has diabetes",
      "B) Tagging an article with multiple topics simultaneously",
  ...
  },
  {
    ...
    options: [
      ...
      "B) No single model is best for every possible problem",
      "C) Training always requires labeled data",
      "D) More data always improves model performance"
    ],
    optionsChinese: [
      "A) 所有模型在所有问题上表现同样好",
      "B) 没有单一模型对每个可能的问题都是最好的",
      "C) 训练总是需要标注数据",
      "D) 更多数据总是能提高模型性能"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "C) Tuning hyperparameters and preventing overfitting",
      "D) Collecting new data samples"
    ],
    optionsChinese: [
      "A) 训练模型权重",
      "B) 提供最终性能指标",
  ...
  },
  {
    ...
    options: [
      ...
      "B) A subset of training samples used in one weight update",
  ...
  },
  {
    ...
    optionsChinese: [
      "A) 参数在训练前设定；超参数在训练中学习",
      "B) 参数在训练中学习；超参数在训练前设定",
      "C) 两者都在训练中学习",
      "D) 两者都在训练开始前设定"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) To convert categorical variables into binary vector representation",
  ...
  },
  {
    ...
    options: [
      ...
      "B) One class has significantly more samples than others",
      "C) The model has too many parameters",
      "D) Training and test sets have different distributions"
    ],
    optionsChinese: [
      "A) 特征具有不同的尺度",
      "B) 一个类别的样本明显多于其他类别",
  ...
  },
  {
    ...
    options: [
      ...
      "B) To maintain the same class distribution in train and test splits",
      "C) To remove outliers from the data",
      "D) To normalize feature values"
    ],
    optionsChinese: [
      "A) 增加数据集的大小",
      "B) 在训练和测试拆分中保持相同的类别分布",
      "C) 从数据中移除异常值",
      "D) 归一化特征值"
    ],
    answer: 1,
  ...
  // ===== LINEAR/LOGISTIC REGRESSION, GRADIENT DESCENT (31-55) =====
  {
    ...
    options: [
      ...
      "B) The linear relationship between input and output with weight w and bias b",
      "C) The gradient descent update rule",
      "D) The probability of class membership"
    ],
    optionsChinese: [
      "A) 要最小化的损失函数",
      "B) 输入和输出之间的线性关系，其中w为权重，b为偏置",
      "C) 梯度下降的更新规则",
      "D) 类别成员的概率"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "D) The training speed of the algorithm"
    ],
    optionsChinese: [
      "A) 模型中使用的特征数量",
      "B) 模型解释的目标中方差的比例",
  ...
  },
  {
    ...
    options: [
      ...
      "B) The relationship is linear and additive",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Binary or multi-class classification problems",
  ...
  },
  {
    ...
    options: [
      ...
      "B) A function that maps input values to a range between 0 and 1",
      "C) A function that calculates the mean of inputs",
      "D) A function that squares the input value"
    ],
    optionsChinese: [
      "A) 输出任意实数的函数",
      "B) 将输入值映射到0和1之间范围的函数",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Batch uses the entire dataset per update; SGD uses one sample per update",
      "C) Both use the entire dataset per update",
      "D) Both use one sample per update"
    ],
    optionsChinese: [
      "A) 批量每次更新使用一个样本；SGD使用整个数据集",
      "B) 批量每次更新使用整个数据集；SGD每次更新使用一个样本",
      "C) 两者每次更新都使用整个数据集",
      "D) 两者每次更新都使用一个样本"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) The model may overshoot the minimum and diverge",
  ...
  },
  {
    ...
    options: [
      ...
      "B) A closed-form analytical solution for finding optimal weights",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Linear regression extended with polynomial features to model non-linear relationships",
      "C) A type of logistic regression",
      "D) Regression that predicts multiple outputs"
    ],
    optionsChinese: [
      "A) 仅使用两个变量的回归",
      "B) 用多项式特征扩展的线性回归以建模非线性关系",
      "C) 一种逻辑回归",
      "D) 预测多个输出的回归"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "A) The maximum number of iterations for training",
      "B) The threshold surface that separates different class predictions",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Preventing exploding gradients by capping gradient values",
  ...
  },
  {
    ...
    options: [
      "A) The initial value of the weights",
      "B) An acceleration technique that accumulates past gradients to speed up convergence",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Convex functions have a single global minimum; non-convex may have multiple local minima",
      "C) Non-convex functions are always smoother",
      "D) There is no practical difference"
    ],
    optionsChinese: [
      "A) 凸函数总是产生更高的损失",
      "B) 凸函数有单个全局最小值；非凸可能有多个局部最小值",
      "C) 非凸函数总是更平滑",
      "D) 没有实际区别"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) The average of absolute differences between predictions and actual values",
  ...
  },
  {
    ...
    options: [
      ...
      "B) It allows the regression line to shift vertically, not passing through the origin",
  ...
  },
  {
    ...
    options: [
      "A) L1 uses absolute errors; L2 uses squared errors",
      "B) L1 uses squared errors; L2 uses absolute errors",
  ...
  // ===== DECISION TREES, RANDOM FORESTS, ENSEMBLE METHODS (56-80) =====
  {
    ...
    options: [
      ...
      "B) By recursively splitting data based on feature thresholds into a tree structure",
      "C) By finding the nearest neighbors",
      "D) By projecting data into a lower-dimensional space"
    ],
    optionsChinese: [
      "A) 通过计算特征的加权和",
      "B) 通过基于特征阈值递归分割数据形成树结构",
      "C) 通过寻找最近邻",
      "D) 通过将数据投影到低维空间"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "A) Measuring the depth of the tree",
      "B) Measuring how mixed the classes are at a node to determine the best split",
      "C) Calculating the learning rate",
      "D) Counting the number of features"
    ],
    optionsChinese: [
      "A) 测量树的深度",
      "B) 测量节点处类别的混合程度以确定最佳分割",
      "C) 计算学习率",
      "D) 计算特征数量"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) Removing branches that provide little predictive power to reduce overfitting",
      "C) Converting the tree into a linear model",
      "D) Increasing the depth of the tree"
    ],
    optionsChinese: [
      "A) 添加更多分支以提高准确度",
      "B) 移除提供很少预测力的分支以减少过拟合",
      "C) 将树转换为线性模型",
      "D) 增加树的深度"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) They can grow deep enough to memorize every training sample",
      "C) They always underfit the data",
      "D) They have too few parameters"
    ],
    optionsChinese: [
      "A) 它们无法捕获非线性关系",
      "B) 它们可以生长得足够深以记忆每个训练样本",
      "C) 它们总是对数据欠拟合",
      "D) 它们参数太少"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) Training models sequentially where each corrects the errors of the previous one",
      "C) Removing poorly performing models from the ensemble",
      "D) Combining unrelated models randomly"
    ],
    optionsChinese: [
      "A) 在随机子集上并行训练模型",
      "B) 顺序训练模型，每个模型纠正前一个的错误",
      "C) 从集合中移除表现差的模型",
      "D) 随机组合不相关的模型"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) By averaging predictions from many diverse trees, reducing variance",
      "C) By using fewer features overall",
      "D) By increasing the bias of the model"
    ],
    optionsChinese: [
      "A) 通过使用更深的树结构",
      "B) 通过对多棵不同树的预测取平均来减少方差",
      "C) 通过总体上使用更少的特征",
      "D) 通过增加模型的偏差"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) The error estimated using samples not included in a tree's bootstrap sample",
      "C) The error after pruning the trees",
      "D) The error on the validation set"
    ],
    optionsChinese: [
      "A) 训练集上的误差",
      "B) 使用未包含在树的自助样本中的样本估计的误差",
      "C) 剪枝后的误差",
      "D) 验证集上的误差"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) A boosting algorithm that assigns higher weights to misclassified samples",
      "C) A dimensionality reduction method",
      "D) A clustering algorithm"
    ],
    optionsChinese: [
      "A) 使用深树的Bagging技术",
      "B) 对错误分类的样本分配更高权重的提升算法",
      "C) 一种降维方法",
      "D) 一种聚类算法"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) Sequentially building trees that fit the residual errors of the previous model",
      "C) Training trees in parallel on random data subsets",
      "D) A method for feature normalization"
    ],
    optionsChinese: [
      "A) 使用梯度下降训练单个模型",
      "B) 顺序构建拟合前一个模型残差的树",
  ...
  },
  {
    ...
    options: [
      ...
      "B) It includes built-in regularization, parallel processing, and handles missing values",
      "C) It does not use decision trees",
      "D) It only works with binary classification"
    ],
    optionsChinese: [
      "A) 它使用更少的树",
      "B) 它包含内置正则化、并行处理并处理缺失值",
      "C) 它不使用决策树",
      "D) 它只适用于二元分类"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) Bagging trains models independently in parallel; boosting trains them sequentially",
      "C) Bagging always outperforms boosting",
      "D) Boosting uses random subsets of features; bagging does not"
    ],
    optionsChinese: [
      "A) Bagging使用神经网络；Boosting使用树",
      "B) Bagging独立并行训练模型；Boosting顺序训练",
      "C) Bagging总是优于Boosting",
      "D) Boosting使用特征的随机子集；Bagging不使用"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "A) A model that achieves 100% accuracy on training data",
  ...
  },
  {
    ...
    options: [
      ...
      "B) A method that combines predictions from multiple models using majority voting or averaging",
      "C) A model that only works with binary classes",
      "D) A technique for data preprocessing"
    ],
    optionsChinese: [
      "A) 输出概率分数的单一模型",
      "B) 使用多数投票或平均来组合多个模型预测的方法",
      "C) 仅适用于二元类别的模型",
      "D) 一种数据预处理技术"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) The maximum number of levels from root to the deepest leaf",
  ...
  },
  {
    ...
    options: [
      "A) The number of features to consider at each split",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Categorical features natively without extensive preprocessing",
      "C) Image data only",
  ...
  },
  {
    ...
    options: [
      ...
      "B) When you want a robust model with less hyperparameter tuning and lower overfitting risk",
      "C) When you have a very small dataset",
      "D) When all features are categorical"
    ],
    optionsChinese: [
      "A) 当你需要不考虑训练时间的最高准确度时",
      "B) 当你想要一个需要更少超参数调整和更低过拟合风险的鲁棒模型时",
      "C) 当你有非常小的数据集时",
      "D) 当所有特征都是分类的时"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) A technique to map data into higher dimensions to find linear separations for non-linear data",
      "C) A way to reduce the number of support vectors",
      "D) A method for data normalization"
    ],
    optionsChinese: [
      "A) 减少训练时间的方法",
      "B) 将数据映射到更高维度以找到非线性数据的线性分离的技术",
      "C) 减少支持向量数量的方法",
      "D) 一种数据归一化方法"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) The regularization parameter that controls the trade-off between margin size and misclassification",
      "C) The kernel type",
      "D) The number of support vectors"
    ],
    optionsChinese: [
      "A) 簇的数量",
      "B) 控制间隔大小和错误分类之间权衡的正则化参数",
      "C) 核类型",
      "D) 支持向量的数量"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "A) A linear kernel for simple problems",
      "B) A kernel that measures similarity based on distance, creating circular decision boundaries",
      "C) A kernel that only works with integer features",
      "D) A kernel that reduces the number of dimensions"
    ],
    optionsChinese: [
      "A) 用于简单问题的线性核",
      "B) 基于距离度量相似性，创建圆形决策边界的核",
      "C) 仅适用于整数特征的核",
      "D) 减少维度数量的核"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) The model becomes sensitive to noise and overfits",
      "C) The model ignores all neighbors",
  ...
  },
  {
    ...
    options: [
      ...
      "B) KNN uses distance metrics, so features with larger scales would dominate the distance calculation",
      "C) It reduces the number of neighbors needed",
      "D) It converts categorical features to numerical"
    ],
    optionsChinese: [
      "A) 它加速训练阶段",
      "B) KNN使用距离度量，因此尺度较大的特征会主导距离计算",
      "C) 它减少所需的邻居数量",
      "D) 它将分类特征转换为数值"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) Because the class decision depends on probability rankings, not exact values",
      "C) Because it always uses all features",
      "D) Because it performs feature selection automatically"
    ],
    optionsChinese: [
      "A) 因为它使用梯度下降进行优化",
      "B) 因为类别决策取决于概率排名而非精确值",
      "C) 因为它总是使用所有特征",
      "D) 因为它自动执行特征选择"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "A) It requires too much training time",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Adding a small count to prevent zero probability for unseen feature values",
      "C) A kernel function for SVM",
  ...
  },
  {
    ...
    options: [
      ...
      "B) When the data is high-dimensional with clear margin separation",
  ...
  },
  {
    ...
    options: [
      ...
      "B) The parameter controlling how far the influence of a single training point reaches",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Using strategies like One-vs-One or One-vs-Rest to decompose into binary problems",
      "C) By increasing the margin width",
      "D) By adding more kernels"
    ],
    optionsChinese: [
      "A) 它无法处理多类问题",
      "B) 使用一对一或一对多等策略分解为二元问题",
      "C) 通过增加间隔宽度",
      "D) 通过添加更多核"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) The initial probability of each class before observing any features",
      "C) The probability of a feature being relevant",
      "D) The error rate of the model"
    ],
    optionsChinese: [
      "A) 看到数据后计算的概率",
      "B) 在观察任何特征之前每个类别的初始概率",
      "C) 特征相关的概率",
      "D) 模型的错误率"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "A) Text classification with word counts",
      "B) Classification when features are continuous and assumed to follow a Gaussian distribution",
      "C) Clustering data into Gaussian-shaped groups",
      "D) Dimensionality reduction"
    ],
    optionsChinese: [
      "A) 使用词频的文本分类",
      "B) 当特征是连续的且假设遵循高斯分布时的分类",
      "C) 将数据聚类成高斯形状的组",
      "D) 降维"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "A) O(1) constant time",
      "B) O(n * d) where n is the number of training samples and d is the number of features",
      "C) O(log n)",
      "D) O(d) where d is the number of features"
    ],
    optionsChinese: [
      "A) O(1)常数时间",
      "B) O(n * d)，其中n是训练样本数，d是特征数",
      "C) O(log n)",
      "D) O(d)，其中d是特征数"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) Adapting SVM to predict continuous values by fitting within an epsilon-insensitive tube",
      "C) Using SVM to reduce data dimensionality",
      "D) A clustering variant of SVM"
    ],
    optionsChinese: [
      "A) 仅将SVM用于分类任务",
      "B) 通过在epsilon不敏感管内拟合来将SVM调整为预测连续值",
      "C) 使用SVM降低数据维度",
      "D) SVM的聚类变体"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) It is extremely fast to train and predict, and works well with small training sets",
      "C) It can model complex non-linear relationships",
      "D) It does not require any assumptions about the data"
    ],
    optionsChinese: [
      "A) 它总是达到更高的准确度",
      "B) 训练和预测速度极快，且在小训练集上表现良好",
      "C) 它可以建模复杂的非线性关系",
      "D) 它不需要对数据做任何假设"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) The proportion of correct predictions out of all predictions",
  ...
  },
  {
    ...
    options: [
      ...
      "B) The proportion of predicted positives that are actually positive (TP / (TP + FP))",
      "C) The total number of correct predictions",
      "D) The inverse of the false negative rate"
    ],
    optionsChinese: [
      "A) 正确预测的实际正样本的比例",
      "B) 预测为正的样本中实际为正的比例 (TP / (TP + FP))",
      "C) 正确预测的总数",
      "D) 假阴性率的倒数"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "A) The proportion of predicted positives that are correct",
      "B) The proportion of actual positives that are correctly identified (TP / (TP + FN))",
      "C) The overall accuracy of the model",
      "D) The ratio of true negatives to total negatives"
    ],
    optionsChinese: [
      "A) 预测为正的样本中正确的比例",
      "B) 正确识别的实际正样本的比例 (TP / (TP + FN))",
      "C) 模型的整体准确度",
      "D) 真阴性与总阴性的比率"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "C) A plot of accuracy vs number of features",
  ...
  },
  {
    ...
    options: [
      ...
      "D) The area under the precision-recall curve only"
    ],
    optionsChinese: [
      "A) 模型的准确度",
      "B) 模型将随机正样本排名高于随机负样本的概率",
      "C) 做出的预测总数",
      "D) 仅精确率-召回率曲线下的面积"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) When there is significant class imbalance in the dataset",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Improving precision often decreases recall and vice versa due to threshold adjustment",
      "C) Precision and recall are always equal",
      "D) The trade-off only exists in regression problems"
    ],
    optionsChinese: [
      "A) 同时提高精确率和召回率总是改善模型",
      "B) 由于阈值调整，提高精确率通常会降低召回率，反之亦然",
      "C) 精确率和召回率总是相等的",
      "D) 这种权衡只存在于回归问题中"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) A plot of precision against recall at different classification thresholds",
      "C) A plot of features vs importance scores",
      "D) A plot of training vs validation accuracy"
    ],
    optionsChinese: [
      "A) 准确度与损失的图",
      "B) 不同分类阈值下精确率与召回率的图",
  ...
  },
  {
    ...
    options: [
      ...
      "B) Failing to predict positive when the actual class is positive",
      "C) Correctly identifying a negative sample",
  ...
  },
  {
    ...
    options: [
      ...
      "B) The unweighted average of F1-scores calculated independently for each class",
      "C) The F1-score of the majority class only",
      "D) The maximum F1-score across all classes"
    ],
    optionsChinese: [
      "A) 在整个数据集上整体计算的F1分数",
      "B) 对每个类别独立计算的F1分数的无加权平均",
      "C) 仅多数类的F1分数",
      "D) 所有类别中的最大F1分数"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) A metric that measures agreement between predictions and actuals while accounting for chance",
      "C) The ratio of precision to recall",
      "D) A measure of model complexity"
    ],
    optionsChinese: [
      "A) 与准确率相同",
      "B) 衡量预测和实际之间一致性同时考虑偶然性的指标",
      "C) 精确率与召回率的比率",
      "D) 模型复杂度的度量"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      ...
      "B) The relative importance of recall versus precision through the beta parameter",
      "C) The classification threshold",
      "D) The number of cross-validation folds"
    ],
    optionsChinese: [
      "A) 特征数量",
      "B) 通过beta参数控制召回率相对于精确率的相对重要性",
      "C) 分类阈值",
      "D) 交叉验证折数"
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      "A) A metric that ranges from 0 to 1",
      "B) A balanced metric ranging from -1 to 1 that considers all four confusion matrix values",
      "C) The correlation between features",
      "D) A metric only used for regression"
    ],
    optionsChinese: [
      "A) 范围从0到1的指标",
      "B) 范围从-1到1的平衡指标，考虑混淆矩阵的所有四个值",
      "C) 特征之间的相关性",
      "D) 仅用于回归的指标"
    ],
    answer: 1,
