...
.ai-modal-carousel .carousel-track {
  position: relative;
  width: 100%;
  overflow: hidden;
  border-radius: 8px;
...
.ai-modal-carousel .carousel-track img {
  max-width: 100%;
  max-height: 200px;
  border-radius: 8px;
  border: 1px solid #0f3460;
...
.ai-modal-carousel .carousel-btn {
  position: absolute;
  top: 50%;
  transform: translateY(-50%);
...
.ai-modal-carousel .carousel-btn:hover {
  background: #45b7d1;
  color: #1a1a2e;
  box-shadow: 0 4px 12px rgba(69, 183, 209, 0.4);
}
.ai-modal-carousel .carousel-btn.prev { left: 8px; }
.ai-modal-carousel .carousel-btn.next { right: 8px; }
.ai-modal-carousel .carousel-dots {
  display: flex;
  justify-content: center;
  gap: 6px;
  margin-top: 8px;
}
.ai-modal-carousel .carousel-dot {
  width: 8px;
  height: 8px;
  border-radius: 50%;
  background: #0f3460;
...
.ai-modal-carousel .carousel-dot.active {
  background: #45b7d1;
  transform: scale(1.2);
}
.ai-modal-carousel .carousel-counter {
  color: #8090b0;
  font-size: 0.75rem;
  margin-top: 4px;
}
.ai-modal-carousel .img-caption {
  color: #8090b0;
  font-size: 0.8rem;
  margin-top: 6px;
}
.ai-modal-carousel .img-caption a {
  color: #45b7d1;
  text-decoration: none;
}
...
.modal-minimized-bar .min-bar-icon {
  font-size: 1rem;
  color: #45b7d1;
}
.modal-minimized-bar .min-bar-title {
  color: #e0e0e0;
  font-size: 0.85rem;
  font-weight: 600;
  white-space: nowrap;
...
.modal-minimized-bar .min-bar-restore {
  background: none;
  border: none;
  color: #4ecca3;
  font-size: 1.1rem;
...
.modal-minimized-bar .min-bar-restore:hover { color: #fff; }
.modal-minimized-bar .min-bar-close {
  background: none;
  border: none;
  color: #a0a0c0;
  font-size: 1.1rem;
...
.term-modal-overlay.maximized .term-modal {
  max-width: 100%;
  width: 100%;
  height: 100vh;
  border-radius: 0;
...
.tutorial-modal-overlay.maximized .tutorial-modal {
  max-width: 100%;
  width: 100%;
  max-height: 100vh;
  height: 100vh;
...
.tutorial-modal-overlay.maximized .tutorial-modal-body {
  flex: 1;
}
...
<div class="term-modal-overlay" id="termModalOverlay" onclick="closeTermModal(event)">
  <div class="term-modal">
    <div class="modal-minmax-btns">
      <button class="modal-minmax-btn btn-minimize" onclick="event.stopPropagation();minimizeModal('term')" title="Minimize">&#x2500;</button>
      <button class="modal-minmax-btn btn-maximize" onclick="event.stopPropagation();toggleMaximizeModal('term')" title="Maximize">&#x2610;</button>
    </div>
...
<div class="ai-modal-overlay" id="aiModalOverlay" onclick="closeAiModal(event)">
  <div class="ai-modal">
    <div class="modal-minmax-btns">
      <button class="modal-minmax-btn btn-minimize" onclick="event.stopPropagation();minimizeModal('ai')" title="Minimize">&#x2500;</button>
      <button class="modal-minmax-btn btn-maximize" onclick="event.stopPropagation();toggleMaximizeModal('ai')" title="Maximize">&#x2610;</button>
    </div>
  ...
  <div class="tutorial-modal">
    <div class="tutorial-modal-header">
      ...
      <div style="display:flex;align-items:center;gap:6px;">
        <button class="modal-minmax-btn btn-minimize" onclick="event.stopPropagation();minimizeModal('tutorial')" title="Minimize">&#x2500;</button>
        <button class="modal-minmax-btn btn-maximize" onclick="event.stopPropagation();toggleMaximizeModal('tutorial')" title="Maximize">&#x2610;</button>
...
function closeTermModal(event) {
  ...
  const bar = document.getElementById('termMinBar');
...
// --- AI Popup History (persistent, max 200) ---
...
function aiHistoryBack() {
  ...
  const imgHtml = urls.length > 0 ? buildImageHtml(urls, entry.imageSearchUrl, entry.displayName) : '';
...
function streamAiQuery(displayName, query) {
  ...
  // Fetch images from Google Image search in parallel
  fetch('index.php?img=' + encodeURIComponent(displayName))
    .then(r => r.json())
    .then(data => {
      const urls = data.imageUrls && data.imageUrls.length > 0 ? data.imageUrls : (data.imageUrl ? [data.imageUrl] : []);
      if (urls.length > 0) {
...
function buildImageHtml(imageUrls, searchUrl, keyword) {
  ...
  const safeSUrl = searchUrl.replace(/"/g, '&quot;');
  ...
  html += '</div>';
  html += '<div class="carousel-counter">' + 1 + ' / ' + imageUrls.length + '</div>';
  html += '<div class="img-caption"><a href="' + safeSUrl + '" target="_blank">View more images for "' + safeKeyword + '"</a></div>';
  html += '</div>';
  return html;
}
...
function carouselNav(carouselId, direction) {
  ...
  carouselGoTo(carouselId, newIdx);
}

function carouselGoTo(carouselId, idx) {
  const carousel = document.getElementById(carouselId);
  if (!carousel) return;
  const imgs = carousel.querySelectorAll('.carousel-img');
...
function closeAiModal(event) {
  ...
  const bar = document.getElementById('aiMinBar');
  if (bar) bar.style.display = 'none';
}
...
function closeTutorial(event) {
  ...
  document.getElementById('tutorialModalOverlay').classList.remove('maximized');
  const bar = document.getElementById('tutorialMinBar');
  if (bar) bar.style.display = 'none';
}
...
(function() {
  ...
  function createMinBar(id, title, restoreFn, closeFn) {
    ...
    bar.className = 'modal-minimized-bar';
    bar.id = id;
    bar.style.display = 'none';
    bar.innerHTML =
      '<span class="min-bar-icon">&#x25A0;</span>' +
      '<span class="min-bar-title">' + title + '</span>' +
      '<button class="min-bar-restore" title="Restore">&#x2610;</button>' +
      '<button class="min-bar-close" title="Close">&times;</button>';
    bar.querySelector('.min-bar-restore').addEventListener('click', function(e) { e.stopPropagation(); restoreFn(); });
    bar.querySelector('.min-bar-close').addEventListener('click', function(e) { e.stopPropagation(); closeFn(); });
    bar.addEventListener('click', restoreFn);
  ...
  window.minimizeModal = function(type) {
    ...
    if (type === 'term') {
      const title = document.getElementById('termModalTitle').textContent || 'Term Details';
      bar.querySelector('.min-bar-title').textContent = title;
      document.getElementById('termModalOverlay').classList.remove('show');
    } else if (type === 'ai') {
      const query = document.getElementById('aiModalQuery').textContent || 'AI Lookup';
      bar.querySelector('.min-bar-title').textContent = query;
      document.getElementById('aiModalOverlay').classList.remove('show');
    } else if (type === 'tutorial') {
      const title = document.getElementById('tutorialModalTitle').textContent || 'Tutorial';
      bar.querySelector('.min-bar-title').textContent = title;
      document.getElementById('tutorialModalOverlay').classList.remove('show');
    }
  ...
  window.restoreModal = function(type) {
    const bar = document.getElementById(type + 'MinBar');
    bar.style.display = 'none';
    if (type === 'term') {
      document.getElementById('termModalOverlay').classList.add('show');
  ...
  window.toggleMaximizeModal = function(type) {
    maximizedState[type] = !maximizedState[type];
    if (type === 'term') {
      document.getElementById('termModalOverlay').classList.toggle('maximized', maximizedState[type]);
    } else if (type === 'ai') {
      const aiModal = document.querySelector('.ai-modal');
      aiModal.classList.toggle('maximized', maximizedState[type]);
      if (maximizedState[type]) {
        ...
        if (saved) {
          try {
            const pos = JSON.parse(saved);
            aiModal.style.left = pos.left + 'px';
            aiModal.style.top = pos.top + 'px';
          } catch(e) {
            const left = Math.max(0, (window.innerWidth - aiModal.offsetWidth) / 2);
            const top = Math.max(0, (window.innerHeight - aiModal.offsetHeight) / 2);
            aiModal.style.left = left + 'px';
            aiModal.style.top = top + 'px';
          }
        }
      }
    } else if (type === 'tutorial') {
      document.getElementById('tutorialModalOverlay').classList.toggle('maximized', maximizedState[type]);
    }
  };
})();
</script>
